...
class Laporan extends CI_Controller
{
    ...
    private function _cetak($data, $table_, $tanggal)
    {
        ...
        if ($table_ == 'barang_masuk') :
            ...
            foreach ($data as $d) {
                ...
                $pdf->Cell(30, 7, $this->admin->joinBarang($d['id_barang']) . ' ' . $d['nama_satuan'], 1, 0, 'C');
                $pdf->Ln();
            }
        else :
            $pdf->Cell(10, 7, 'No.', 1, 0, 'C');
            ...
            foreach ($data as $d) {
                ...
                $pdf->Cell(30, 7, $this->admin->stokById('detail_barang', 'BK', $d['id_barang']) . ' ' . $d['nama_satuan'], 1, 0, 'C');
                $pdf->Ln();
            }
        endif;
    ...
    {
        $table = 'barang_masuk';
        $query = $this->admin->getBarangMasukAlll(null, null, null);
        $this->cetakDataAll($query, $table);
    }

    private function cetakDataAll($data, $table_)
    {
        $this->load->library('CustomPDF');
        $table = $table_ == 'barang_masuk' ? 'Barang Masuk' : 'Barang Keluar';
        ...
        if ($table_ == 'barang_masuk') :
            ...
            foreach ($data as $d) {
                ...
                $pdf->Cell(25, 7, $d['id_detail_barang'], 1, 0, 'L');
                $pdf->Cell(40, 7, $d['nama_barang'], 1, 0, 'L');
                $pdf->Cell(30, 7, $d['aksi'], 1, 0, 'C');
                $pdf->Ln();
            }
        endif;
    ...
    public function cetakDetail()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $query = '';
            if ($table == 'barang_masuk') {
                $query = $this->admin->getDetailBarangMasuk($id_barang);
            } else {
                $query = $this->admin->getDetailBarangKeluar($id_barang);
            }

            $this->detailCetak($query, $table);
        }
    }

    private function detailCetak($data, $table_)
    {
        $this->load->library('CustomPDF');
        $table = $table_ == 'barang_masuk' ? 'Barang Masuk' : 'Barang Keluar';
